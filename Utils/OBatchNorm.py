...
class BatchNorm(Layer):
  """A slight modification of Batch normalization layer (Ioffe and Szegedy, 2014).
    ...
    stride: samples the input values every stride pixels in the height
    and width dimension for calculation of mean and standard deviation
  ...
  ###############################################################################
  ####### ADDING STRIDES IN THE INPUT WHILE CALCULATING MEAN AND VARIANCE #######
  ###############################################################################
  def _moments(self, inputs, reduction_axes, keep_dims):
    mean, variance = nn.moments(inputs[:, ::self.stride, ::self.stride, :],
  ...
  def call(self, inputs, training=None):
    ...
    ####################################################
    ######## ORIGINAL BATCH NORMALIZATION LOGIC ########
    ####################################################
    ...
    ####################################################
    ######## MODIFIED BATCH NORMALIZATION LOGIC ########
    ####################################################
    # mean = math_ops.cast(mean, inputs.dtype)
    # variance = math_ops.cast(variance, inputs.dtype)
    # training_normed_input = (inputs - _broadcast(mean)) * math_ops.rsqrt(_broadcast(variance) + self.epsilon)
    # outputs = DepthwiseConv2D(kernel_size = 1, padding='same', depthwise_initializer='ones',
    #         use_bias=True, bias_initializer = 'zeros')(training_normed_input)
    # # If some components of the shape got lost due to adjustments, fix that.
    # outputs.set_shape(input_shape)
    #     # return outputs
    ####################################################
