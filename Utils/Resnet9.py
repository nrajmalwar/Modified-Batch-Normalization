...
def Resnet9(input_dim, n_classes, hparams, obn=False, mbn=False, stride=1): 
    ...
    def conv_bn_relu(x, layer_name, pool=False):
        ...
        # If obn is true apply Original Batch Normalization
        if obn == True:
            x = OBatchNorm(momentum=0.9, epsilon=1e-5, stride=stride)(x)
        # Else apply Modified Batch Normalization
        else:
            x = MBatchNorm(stride=stride)(x)
